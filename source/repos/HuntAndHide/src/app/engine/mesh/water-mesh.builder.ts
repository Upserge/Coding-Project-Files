...
function buildShoreRing(radius: number): THREE.Mesh {
  const geo = new THREE.RingGeometry(radius, radius + 0.5, 24);
  const mat = new THREE.MeshStandardMaterial({
    color: SHORE_COLOR,
    roughness: 1,
    side: THREE.DoubleSide,
...
function buildStreamSurface(length: number): THREE.Mesh {
  const geo = new THREE.PlaneGeometry(1.8, length, 8, 16);
  const mesh = new THREE.Mesh(geo, createWaterMaterial());
  mesh.rotation.x = -Math.PI / 2;
  mesh.position.y = 0.03;
  return mesh;
...
function buildStreamBed(length: number): THREE.Mesh {
  const geo = new THREE.PlaneGeometry(2.2, length + 0.6, 1, 4);
...
function addStreamPebbles(group: THREE.Group, length: number): void {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    pebble.scale.setScalar(0.5 + Math.random() * 0.8);
    group.add(pebble);
  }
}
